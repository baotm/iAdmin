...
export default {
  ...
  async fetch () {
    ...
    this.id = id;
    let listQuery = [
      this.$strapi.$trackings.find({ hopdong_id: id }),
      this.$strapi.$hopdongs.findOne(id),
      await this.$strapi.$chungtus.find({ hopdong_id: id }),
      await this.$strapi.$nhatkihopdongs.find({ hopdong_id: id })]
    Promise.all(listQuery).then(values => {
      this.tracking = values[0]
      this.hopdong = values[1]
      this.chungtu = values[2]
      this.nhatkilai = values[3]
      this.dataReady = true;
    });
  ...
  methods: {
    ...
    chuocHopDong () {
      ...
      let mes = {
        title: 'Chuộc hợp đồng',
        body: 'Bạn muốn chuộc hợp đồng'
      }
      mes.body = `
      <br/>
      <table class="table">
...
<tbody>
<tr>
    <td><strong>Tiền Gốc</strong> </td>
    <td>${this.numberWithCommas(tiengoc)}</td>
  </tr>
  <tr>
    <td><strong>Tiền Lãi</strong></td>
    <td>${this.numberWithCommas(tienlai)}</td>
  </tr>
  <tr>
    <td><strong>Tiền Nợ</strong</td>
    <td>${this.numberWithCommas(tienno)}</td>
  </tr>
  <tr>
    <td><strong>Tổng</strong></td>
<td>${this.numberWithCommas(tientong)}</td>
  </tr>
</tbody>
</table>`

      this.$dialog.confirm(mes).then(async () => {
        let hopdong1 = {
          tinhtrang: 'DACHUOC',
          tienno: 0
        }
        await this.$strapi.$hopdongs.update(this.id, hopdong1)
        mes.body = "Cập Nhật Chứng Từ Về Việc Nhận Tiền Vốn Và Lãi Của Hợp Đồng"
        //tao chungtu
        let data_chungtu = {
          sotien: tientong,
        ...
        mes.body = "Cập Nhật Cập Nhật Xuất Kho Của Hợp Đồng"
        ...
        mes.body = "Lưu Thông Tin Truy Cập CHo Hợp Đồng"
        let nhatkihopdong = {
          Ten: "CHUOCDO",
          SoTien: tientong,
          account: this.getInfoAccount()
        ...
        mes.body = "Gửi Thông Báo Cho Admin"
    ...
    xoaHopDong () {
      let mes = {
        title: 'Thông báo',
        body: `<span style='color:red'>Bạn muốn xóa hợp đồng này</span>`
      }
      this.$dialog.confirm(mes).then(async () => {
        await this.$strapi.hopdongs.delete(this.id)
        window.location.replace('/hopdong')
      })
    },
    formatDay (x) {
